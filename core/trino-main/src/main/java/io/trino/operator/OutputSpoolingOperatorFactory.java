        ...
        implements OperatorFactory
{
    ...
    public static Map<Symbol, Integer> spooledLayout(Map<Symbol, Integer> layout)
    {
            ...
            implements Operator
    {
        ...
        {
            long rows = reduce(pages, page -> (long) page.getPositionCount());
            if (finished) {
            ...
            try (OutputStream output = spoolingManager.createOutputStream(segmentHandle); ByteArrayOutputStream bufferedOutput = new ByteArrayOutputStream(toIntExact(controller.getCurrentSpooledSegmentTarget()))) {
                ...
                controller.recordEncoded(attributes.get(BYTE_SIZE, Integer.class));
                return emptySingleRowPage(layout, new SpooledBlock(spoolingManager.handleToUriIdentifier(segmentHandle), attributes).serialize());
            }
            catch (Exception e) {
        ...
        static long reduce(List<Page> page, Function<Page, Long> reduce)
        {
            return page.stream().map(reduce).reduce(0L, Long::sum);
        }
    ...
    private record PageBuffer(LinkedList<Page> buffer)
    {
        private PageBuffer
        {
        ...
        {
            return new PageBuffer(new LinkedList<>());
        }
            ...
            implements Supplier<OutputSpoolingInfo>
    {
        ...
        public OutputSpoolingInfo get()
        {
            return new OutputSpoolingInfo(
                    new Duration(encodingTiming.getWallNanos(), NANOSECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(encodingTiming.getCpuNanos(), NANOSECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(spoolingTiming.getWallNanos(), NANOSECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(spoolingTiming.getCpuNanos(), NANOSECONDS).convertToMostSuccinctTimeUnit(),
            ...
            implements Mergeable<OutputSpoolingInfo>, OperatorInfo
    {
        ...
        {
            return new OutputSpoolingInfo(
                    new Duration(encodingWallTime.toMillis() + other.encodingWallTime().toMillis(), MILLISECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(encodingCpuTime.toMillis() + other.encodingCpuTime().toMillis(), MILLISECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(spoolingWallTime.toMillis() + other.spoolingWallTime().toMillis(), MILLISECONDS).convertToMostSuccinctTimeUnit(),
                    new Duration(spoolingCpuTime.toMillis() + other.spoolingCpuTime().toMillis(), MILLISECONDS).convertToMostSuccinctTimeUnit(),
