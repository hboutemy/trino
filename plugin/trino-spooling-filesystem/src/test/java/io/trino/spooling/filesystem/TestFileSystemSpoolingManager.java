...
public class TestFileSystemSpoolingManager
{
    ...
    {
        FileSystemSpooledSegmentHandle handle = new FileSystemSpooledSegmentHandle("test", Date.from(Instant.now()));
        assertThat(getSpoolingManager().deserialize(getSpoolingManager().serialize(handle))).isEqualTo(handle);
    }
